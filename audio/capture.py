...
from audio.constants import INT16_MAX, INT16_MIN
...
class AudioCapture:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            logger.info(
                ...
                self.device_id, self.sample_rate, self.sensitivity,
            )
    ...
    def read_chunk(self, on_level: Callable[[float], None] | None = None) -> bytes | None:
        """
    ...
    def _apply_gain(self, raw: bytes) -> bytes:
        ...
        scaled = [
            max(INT16_MIN, min(INT16_MAX, int(s * self.sensitivity)))
            for s in samples
        ]
