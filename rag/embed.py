...
def resolve_embedding_model(base_url: str, configured: str, timeout_sec: float = 10.0) -> str:
    """
    ...
    if not available:
        raise ValueError(
            "Ollama not reachable or returned no models. Ensure Ollama is running. " + PULL_MESSAGE
        )
    ...
    for candidate in EMBEDDING_MODEL_FALLBACKS:
        if candidate in available:
            logger.info("Using embedding model %s (configured %s not found)", candidate, configured)
            return candidate
...
class OllamaEmbedClient:
    ...
    def embed(self, inputs: str | list[str]) -> list[list[float]]:
        ...
        payload: dict[str, Any] = {"model": model, "input": inputs if len(inputs) > 1 else inputs[0]}
